...
[`vignette("layout")`](layout.md) for facets and grouped category
axes, [`vignette("boxplots")`](boxplots.md) for the box-plot styles
...
set. Growth rates span zero, so the black zero line appears by itself:
...
holds a handful of ordered categories – age brackets, quintiles – the
house style puts a marker on every observation, so the reader can see
where the measurements actually are. `points = TRUE` adds them in the
line colour, and the legend key becomes a line with a marker on it:
...
too heavy against a thicker line.
...
show (say) 2021 above 2024 you reverse the `jaar` levels and let
...
Two series belong in one figure when they are about the same thing but
an order of magnitude apart – a wealth total in billions, and the tax
raised on it. `sec_y` draws the second series as a line against a
...
which axis it belongs to. The combination is refused where it cannot be
read: horizontal bars, grouped categories, and `position = "fill"` all
error rather than draw something misleading.
...
`cpb_dot()` draws the coefficient-plot form used for regression output:
one estimate per row with its interval through it, read against a black
line at zero. It is horizontal by default, so the rows read top to
bottom in the order of the factor levels. `group` collects the rows
...
specifications side by side, and use `cap_width = 0` for plain intervals
without end caps.
...
Every data set above is **simulated** and every label is a placeholder –
nothing here reproduces a published number. What the examples do follow
is the *layout* of real CPB figures, so that the house style has
something concrete to be judged against.
...
- [Erfbelasting in beeld: feiten, percepties en voorkeuren van
  ...
  – the layouts behind *Voorkeurstarieven erfbelasting naar leeftijd*
  (markers on the line), *Belasting over erfenissen* (secondary axis)
  and *Samenhang voorkeurstarieven, percepties en kenmerken*
  (dot-and-interval).
