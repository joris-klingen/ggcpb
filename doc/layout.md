...
The chart types in `vignette("chart-types")` each show a single chart.
...
Where facets split the figure into self-contained panels, `group` keeps
everything on *one* comparable value axis. Pass it to `cpb_col()` to
organise the categories into blocks – a gap between the groups, the
group names in bold under the category labels:
...
`box_style = "james"` (see `vignette("boxplots")` for the styles) and a
...
canvas – here it is drawn 4.5 in tall. For grouped boxes *with a fill
mapping* (e.g. two dodged years per category), see
`vignette("boxplots")`.
