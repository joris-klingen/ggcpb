...
`legend_ncol`. `drop = FALSE` keeps every class in the legend even when
no observation falls in it, so the reader sees the full scale rather
than silently losing a class.
