...
# wide; text sizes in theme_cpb() are absolute points, so any other
# canvas distorts the text-to-figure proportion) and only *displayed*
# larger via out.width.
...
extended further with `+`. The figures use `style = "nplot"`, the
house look of published CPB figures; leave `style` unset for the
lighter hand-rolled ggplot look (see the last section, and
`vignette("ggcpb")` for the composable core the wrappers build on).
...
* **Zero line.** Under `style = "nplot"` a solid black zero line is
  ...
  (which are anchored at zero), and for lines and boxplots whenever
  the data spans zero.
...
labels, `value_limits` fixes its range, and under `style = "nplot"`
the panel starts exactly at the zero axis:
...
```{r box-single, fig.width = 5.96, fig.height = 3.2, out.width = "100%"}
...
```{r box-dodged, fig.width = 5.96, fig.height = 2.98, out.width = "100%"}
...
Everything above used `style = "nplot"`. The default
`style = "ggplot"` keeps the lighter hand-rolled look -- CPB-grey
gridlines with minors, no ticks or zero line, legend on the right:
...
`grid_colour`, `flush_legend`, `zeroline`, ...) can be set per call,
see `?theme_cpb`.
